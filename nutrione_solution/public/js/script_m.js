var historyCnt = 0;
var popCnt = 0;
var headBannerH;

var mSubScript = (function () {
	var device = '';
	var browser = '';

	return {
		checkEvt: function () {
			//디바이스, 브라우저 체크이벤트
			if (/Android/i.test(navigator.userAgent)) {
				device = "android";
			} else if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
				return navigator.userAgent.match(/(iPhone|iPod)/g) ? device = "ios" : device = "ipad";
			} else {
				device = "pc";
			}

			var agent = navigator.userAgent.toLowerCase(),
				name = navigator.appName;

			if (name === "Microsoft Internet Explorer" || agent.indexOf("trident") > -1 || agent.indexOf("edg/") > -1) {
				browser = "ie";
				if (name === "Microsoft Internet Explorer") { // IE old version (IE 10 or Lower)
					agent = /msie ([0-9]{1,}[\\.0-9]{0,})/.exec(agent);
					browser += parseInt(agent[1]);
				} else { // IE 11+
					if (agent.indexOf("trident") > -1) { // IE 11
						browser += 11;
					} else if (agent.indexOf("edg/") > -1) { // Edge
						browser = "edge";
					}
				}
			} else if (agent.indexOf("safari") > -1) { // Chrome or Safari
				if (agent.indexOf("opr") > -1) { // Opera
					browser = "opera";
				} else if (agent.indexOf("chrome") > -1) { // Chrome
					browser = "chrome";
				} else { // Safari
					browser = "safari";
				}
			} else if (agent.indexOf("firefox") > -1) { // Firefox
				browser = "firefox";
			}
		},

		selectEvt : function(){
			//select_form
			$(document).on("click", ".select_form .select", function(){
				if(!$(this).parents(".select_form").hasClass("disabled")) {
					if (!$(this).parents(".select_form").hasClass("on")) {
						$(this).parents(".select_form").addClass("on");
						$(this).parents(".add_select").addClass("on");
						$(this).parents(".select_form").find(".option").stop(true, true).slideDown(200);
					} else {
						$(this).parents(".select_form").find(".option").stop(true, true).slideUp(200, function () {
							$(this).parents(".select_form").removeClass("on");
							$(this).parents(".add_select").removeClass("on");
						});
					}
				}
			})
			$(document).on("click", ".select_form .option a", function(){
				if(!$(this).hasClass("disabled")) {
					$(this).parents(".select_form").find(".select").attr("value", $(this).find(".text span").attr("value"));
					$(this).parents(".select_form").find(".select p").text($(this).find(".text span b").text());
					if ($(this).parents(".select_form").hasClass("count")) {
						$(this).parents(".select_form").find(".count_txt").show();
					}
					if ($(this).hasClass("textarea")) {
						$(this).parents(".input_form").find(".textarea_wrap").parents(".input_area").show();
					} else {
						$(this).parents(".input_form").find(".textarea_wrap").parents(".input_area").hide();
					}
					$(this).parents(".select_form").find(".select span").show();
					$(this).parents(".select_form").find(".option").stop(true, true).slideUp(200, function () {
						$(this).parents(".select_form").removeClass("on");
						$(this).parents(".add_select").removeClass("on");
					});
				}
			});

			$("body").click(function(e) {
				if ($(".select_form.on .option").is(":visible")) {
					if (!$(".select_form.on").has(e.target).length) {
						$(".select_form.on .option").stop(true, true).slideUp(200, function () {
							$(this).parents(".select_form").removeClass("on");
							$(this).parents(".add_select").removeClass("on");
						});
					}
				}
			});
		},

		tabEvt: function () {
			//탭이벤트
			$(document).on("click", ".cont_tab a", function () {
				if (!$(this).parents(".cont_parent").size() > 0) {
					//페이지 내에 탭이 여러개 있어서 부모로 묶어서 구분 지어야할때
					if (!$(this).hasClass("on")) {
						// 탭
						$(this).siblings("a").removeClass("on");
						$(this).addClass("on");
						$(this).parents(".cont_parent").find(".cont_con").eq($(this).parents(".cont_parent").find(".cont_tab a").index($(this))).siblings(".cont_con").css("display", "none");
						$(this).parents(".cont_parent").find(".cont_con").eq($(this).parents(".cont_parent").find(".cont_tab a").index($(this))).css("display", "block");
					}
				} else {
					//페이지 내에 탭이 a태그로 구현되는 탭이 하나라 부모 태그 없어도 될때
					if (!$(this).hasClass("on")) {
						// 탭
						$(this).siblings("a").removeClass("on");
						$(this).addClass("on");
						$(".cont_con").eq($(this).index()).siblings(".cont_con").css("display", "none");
						$(".cont_con").eq($(this).index()).css("display", "block");
					}
				}
			});
			
			//라디오버튼 탭
			$(document).on("click", ".radio_tab .radio_box input", function () {
				if (!$(this).parents(".radio_box").hasClass("on")) {
					$(this).parents(".radio_tab").find(".radio_box").removeClass("on");
					$(this).parents(".radio_box").addClass("on");
					$(this).parents(".radio_tab_parents").find(".radio_tab_con .tab_cont").css("display", "none");
					$(this).parents(".radio_tab_parents").find(".radio_tab_con .tab_cont").eq($(this).parents(".radio_box").index()).css("display", "block");
				}
			});
			
			if($(".head_banner").is(":visible")){
				headBannerH = $(".head_banner").outerHeight();
			}else{
				headBannerH = 0;
			}

			if ($(".scroll_tab_parent").size() > 0){
				var anchorTop = 0;
				// 앵커 탭
				$(document).on("click", ".scroll_tab_parent .cont_tab a", function () {
					anchorTop = $(".scroll_tab_parent .cont_con").eq($(this).index()).offset().top - $(".cont_tab").outerHeight() - 10;
					if ($(".indicator").size() > 0){
						anchorTop = anchorTop - $(".indicator").outerHeight() - headBannerH;
					}else{
						anchorTop = anchorTop - $("header").outerHeight() - headBannerH;
					}
					TweenMax.to($("html, body"), 0.4, {scrollTop: anchorTop});
				});
			}

		},

		accorEvt: function () {
			//아코디언 이벤트
			$(document).on("click", ".accor_div .list .open", function () {
				//아코디언 이벤트, each클래스 있을시 다른 아코디언과 관계 없이 움직임. each클래스 없을시 다른 아코디언 열릴시 닫힘.
				if (!$(this).parents(".list").hasClass("on")) {
					if (!$(this).parents(".accor_div").hasClass("each")) {
						$(this).parents(".accor_div").find(".list").removeClass("on");
						$(this).parents(".accor_div").find(".list .close").stop(true, true).slideUp(300);
					}
					$(this).parents(".list").addClass("on");
					$(this).parents(".list").find(".close").stop(true, true).slideDown(300)
					if($(this).parents(".list").find(".close").find(".swiper-container").size() > 0){
						setTimeout(() => {
							afterResize(".accor_div .list .close .swiper-container .img_resize")
						}, 100);
					}
				} else {
					$(this).parents(".list").removeClass("on");
					$(this).parents(".list").find(".close").stop(true, true).slideUp(300);
				}
			});
		},

		resizeEvt: function () {
			$(window).on("resize", function () {
				//이미지 리사이징
				$(".img_resize").each(function () {
					if ($(this).find("img").size() > 0) {
						if($(this).parents(".swiper-container").size() > 0){
							if ($(this).find("img").get(0).naturalWidth * $(this).height() / ($(this).find("img").get(0).naturalHeight * $(this).width()) >= 1){
								$(this).addClass("reverse");
							}
						}else{
							if ($(this).find("img").get(0).naturalWidth * $(this).height() / ($(this).find("img").get(0).naturalHeight * $(this).width()) <= 1){
								$(this).addClass("reverse");
							}
						}
					}
				})
			});
			$(window).resize();
		},

		scrollEvt: function () {
			var prevScroll = $(window).scrollTop();

			$(window).on("scroll", function () {
				if($(".head_banner").is(":visible")){
					headBannerH = $(".head_banner").outerHeight();
				}else{
					headBannerH = 0;
				}

				//헤더이벤트
				if ($(window).scrollTop() > 48) {
					if ($(window).scrollTop() > prevScroll) {
						//하단
						$("header").removeClass("up");
						$("header").css({"position":"","top":""});
						if(!$(".indicator").hasClass("item_detail")) {
							$(".indicator").removeClass("up");
							$(".indicator").css({"position": "", "top": ""});
						}
						if($(".head_banner").is(":visible")){
							$("header").css("top", "-" +( 48 - headBannerH));
						}else{
							$("header").css({"position": "", "top": ""});
						}
					} else {
						$("header").addClass("up");
						if($(".head_banner").is(":visible")){
							$("header").css("top", headBannerH);
						}else{
							$("header").css({"position": "", "top": ""});
						}
						if(!$(".indicator").hasClass("item_detail")) {
							$(".indicator").addClass("up");
							if($(".head_banner").is(":visible")){
								$(".indicator").css({"position": "", "top": headBannerH});
							}else{
								$(".indicator").css({"position": "", "top": ""});
							}
						}else{

						}
					}
				} else {
					if ($(window).scrollTop() > prevScroll) {
						$("header").removeClass("up");
						if(!$(".indicator").hasClass("item_detail")) {
							$(".indicator").removeClass("up");
							$(".indicator").css({"position": "absolute", "top": "headBannerH"});
						}
						$("header").css({"position":"absolute", "top": headBannerH});
					}else{
						$("header").addClass("up");
						if($(".head_banner").is(":visible")){
							$("header").css("top", headBannerH);
						}else{
							$("header").css({"position":"","top":""});
						}
						if(!$(".indicator").hasClass("item_detail")) {
							$(".indicator").addClass("up");
							$(".indicator").css({"position": "", "top": headBannerH});
						}
					}
				}


				if ($(window).scrollTop() >= 100) {
					if ($(window).scrollTop() > prevScroll) {
						$(".floating_btn .top_btn").hide();
					}else{
						$(".floating_btn .top_btn").show();
					}
				}else{
					$(".floating_btn .top_btn").hide();
				}

				

				//상품상세 앵커 스크롤
				if($(".item_dtl_div .item_dtl_tab_area .anchor_tab").size() > 0){
					if($(window).scrollTop() + $(".indicator").outerHeight() + headBannerH >= $(".item_dtl_div .item_dtl_tab_area").offset().top){
						$(".item_dtl_div .item_dtl_tab_area .anchor_tab").addClass("fixed");
						$(".item_dtl_div .item_dtl_tab_area .anchor_tab").css({"top": $(".indicator").outerHeight() + headBannerH});
						$(".item_dtl_div .item_dtl_tab_area").css({"padding-top": $(".indicator").outerHeight()});
					}else{
						$(".item_dtl_div .item_dtl_tab_area .anchor_tab").removeClass("fixed");
						$(".item_dtl_div .item_dtl_tab_area .anchor_tab").css({"top":""});
						$(".item_dtl_div .item_dtl_tab_area").css({"padding-top": ""});
					}

					$(".item_dtl_div .item_dtl_tab_area .anchor_tab a").each(function(q){
						if($(window).scrollTop() >= $(".tab_con").eq(q).offset().top - $(".item_dtl_div .item_dtl_tab_area .anchor_tab").outerHeight() - $(".indicator").outerHeight() - 10 - headBannerH){
							if (!$(".item_dtl_div .item_dtl_tab_area .anchor_tab a").eq(q).hasClass("on")) {
								$(".item_dtl_div .item_dtl_tab_area .anchor_tab a").removeClass("on");
								$(".item_dtl_div .item_dtl_tab_area .anchor_tab a").eq(q).addClass("on");
							}
						}
					})
				}

				prevScroll = Math.abs($(window).scrollTop());
			});
			$(window).scroll();

		},

		etcEvt: function () {
			// 띠배너
			setTimeout(() => {
				if($("#wrap").parents(".main").size() > 0){
					contentPaddingTop = parseInt($("#wrap.main #content").css("padding-top"));
				}else{
					contentPaddingTop = parseInt($("#content").css("padding-top"));
				}
			}, 100);
			
			if($(".head_banner").size() > 0){
				headBannerH = $(".head_banner").outerHeight();
			}else{
				headBannerH = 0;
			}

			if($(".head_banner").is(":visible")){
				setTimeout(() => {
					$("header").css("top", headBannerH);
					$(".indicator").css("top", headBannerH);
					$("#content").css("padding-top", contentPaddingTop + headBannerH);
				}, 100);
			}else{
				$("header").css("top", "");
				$(".indicator").css("top", "");
				$("#content").css("padding-top", "");
			}


			$(".head_banner .close_btn").click(function (){
				if ($("header.up").size() > 0){
					$("header").css("top", "0");
					TweenMax.to($(this).parents(".head_banner"), 0.4, {top:-headBannerH, onComplete:function(){
							$(".head_banner").hide();
						}});
					$("#content").css("padding-top", "");
				}
				if($(".indicator.up").size() > 0){
					$(".indicator").css("top", "0");
					$("#content").css("padding-top", "");
					TweenMax.to($(this).parents(".head_banner"), 0.4, {top:-headBannerH, onComplete:function(){
						$(".head_banner").hide();
					}})
					if($(".anchor_tab.fixed").size() > 0){
						$(".anchor_tab.fixed").css("top", $(".indicator").outerHeight());
					}
				}else{
					$(this).parents(".head_banner").hide();
					$(".indicator").css("top", "");
					$("#content").css("padding-top", "");
				}					
			});

			//전체메뉴
			var isClose = true;
			$(".allmenu_btn").click(function () {
				isClose = true;
				history.pushState(null, document.title, location.href);
				historyCnt ++;
				scrollT = $(window).scrollTop();
				$("html").addClass("lock");
				$("header").removeClass("up");
				$(".dimd_bg").stop(true, true).fadeIn(300);
				TweenMax.to($(".allmenu_pop"), 0.4, {left: 0, ease: Power3.easeOut});

				//뒤로가기
				window.onpopstate = function(event){
					popCnt ++;
					if ($(".allmenu_pop").is(':visible')){
						TweenMax.to($("header"), 0, {className: "+=up", delay: 0.01});
						if(isClose) {
							$("html").removeClass("lock");
							$("html, body").scrollTop(scrollT);
							$(".dimd_bg").stop(true, true).fadeOut(300);
							TweenMax.to($(".allmenu_pop"), 0.4, {left: "-100%", ease: Power3.easeOut});
						}
						history.scrollRestoration = 'manual';
					}else{
						history.go(-(historyCnt - popCnt + 1));
					}
				}
			});
			$(".allmenu_pop .close_btn").click(function () {
				$("html").removeClass("lock");
				$("html, body").scrollTop(scrollT);
				$(".dimd_bg").stop(true, true).fadeOut(300);
				TweenMax.to($("header"), 0, {className: "+=up", delay: 0.01});
				TweenMax.to($(".allmenu_pop"), 0.4, {left: "-100%", ease: Power3.easeOut});
				history.back();

				//메뉴 닫을 때 언어선택 select 열려있으면 닫기!
				if($(".allmenu_pop .utill_div .lang_div .select").hasClass("on")){
					$(".allmenu_pop .utill_div .lang_div .select").click();
				}
			});
			
			$(".allmenu_pop nav .menu").each(function(){
				if($(this).find(".two_depth").size() > 0){
					$(this).addClass("has_two");
				}
			});

			$(".allmenu_pop nav .menu.has_two .one_depth").click(function(){
				if (!$(this).hasClass("on")) {
					if($(this).parents(".allmenu_pop").hasClass("each_depth")){
						$(this).parents(".menu").siblings().find(".one_depth").removeClass("on");
						$(this).parents(".menu").siblings().find(".two_depth").stop(true, true).slideUp(300);
					}
					$(this).addClass("on");
					$(this).parents(".menu").find(".two_depth").stop(true, true).slideDown(300);
				} else {
					$(this).removeClass("on");
					$(this).parents(".menu").find(".two_depth").stop(true, true).slideUp(300);
				}
			})

			$(".allmenu_pop nav .two_depth a").click(function(){
				$(this).addClass("on");
			})

			//전체메뉴 > 언어선택 셀렉트
			$(".allmenu_pop .utill_div .lang_div .select").click(function(){
				if($(this).hasClass("on")){
					$(this).removeClass("on");
					TweenMax.to($(".allmenu_pop .utill_div .lang_div .option"), 0.2, {top: "0", display:"none", opacity:"0", ease: Power3.easeOut});
				}else{
					$(this).addClass("on");
					TweenMax.to($(".allmenu_pop .utill_div .lang_div .option"), 0.2, {top: "-24px", display:"block", opacity:"1", ease: Power3.easeOut});
				}
			})
			
			// #content 높이가 VH보다 길 때 밑에 고정하는 버튼
			if($(".btn_type.scroll_float").size() > 0){
				if($("#content").outerHeight() >= $(window).outerHeight()){
					$(".btn_type.scroll_float").addClass("fixed");
				}
			}

			//floating
			if($(".floating_btn .cs_btn_wrap").size() > 0){
				$(".floating_btn .cs_btn_wrap .cs_btn").click(function(){
					if($(this).parents(".cs_btn_wrap").hasClass("on")){
						$(".floating_btn .cs_btn_wrap").removeClass("on");
						$(".dimd_bg").stop(true, true).fadeOut(300);
					}else{
						$(".floating_btn .cs_btn_wrap").addClass("on");
						$(".floating_btn .cs_btn_wrap .cs_btn_cont").css({"top":-$(".floating_btn .cs_btn_wrap .cs_btn_cont").outerHeight() - 12});
						$(".dimd_bg").stop(true, true).fadeIn(300);
					}
				})
			}
			$(".floating_btn .top_btn").click(function(){
				TweenMax.to($("html, body"), 0.5, {scrollTop:0})
			})


			var scrollT = 0;
			//bottom_fix 버튼
			if ($(".bottom_fix").is(":visible") > 0) {
				$("#wrap").addClass("bottom");
			}

			if ($(".item_option_layer").is(":visible") > 0) {
				$("#wrap").addClass("bottom");
			}

			//레이어팝업 하단고정버튼
			$(".layer_pop").each(function () {
				if ($(this).find(".bottom_fix").size() > 0) {
					$(this).addClass("has_bottom_fix");
				}
			})

			//인풋 + 버튼
			$(".input_form .input_area").each(function () {
				if ($(this).find(".btn").size() > 0) {
					$(this).addClass("has_btn");
				}

				if($(this).hasClass("pw_view")){
					$(this).find(".view").click(function(){
						if(!$(this).hasClass("on")){
							$(this).addClass("on");
							$(this).parents(".input_area").find("input").attr("type","text");
						}else{
							$(this).removeClass("on");
							$(this).parents(".input_area").find("input").attr("type","password");
						}
					})
				}
			})

			//체크박스 이벤트
			$(".chk_box input").each(function () {
				$(this).click(function () {
					if(!$(this).parents(".chk_box").hasClass("fix")) {
						//.chk_box에 fix 클래스 넣을시 이벤트 작동 안하게 만들 수 있음.
						if (!$(this).parents(".chk_box").hasClass("on")) {
							$(this).parents(".chk_box").addClass("on");
							$(this).prop("checked", true);
						} else {
							$(this).parents(".chk_box").removeClass("on");
							$(this).prop("checked", false);
						}
					}
				})
			});
			
			// 체크박스 전체 동의
			$(".all_chk_container .chk_box input").each(function(){
				$(this).click(function(){
					if($(this).parents(".chk_div").hasClass("all_chk_div")){				
						// 전체동의 눌렀을 때
						if($(this).parents(".chk_box").hasClass("on")){
							// 전체동의 꺼져있을 때
							$(this).parents(".all_chk_container").find("[class*='chk']").addClass("on");
						}else{
							// 전체동의 켜져있을 때
							$(this).parents(".all_chk_container").find("[class*='chk']").removeClass("on");
						}
					}else{
						//다른 거 눌렀을 때
						if(!$(this).parents(".all_chk_container").find(".all_chk_div .chk_box").hasClass("on")){
							// 전체동의 꺼져있을 때
							if($(this).parents(".all_chk_container").find(".chk_box").size()-1 == $(this).parents(".all_chk_container").find(".chk_box.on").size()){
								$(this).parents(".all_chk_container").find(".all_chk_div, .all_chk_div *").addClass("on");
							}
						}else{
							// 전체동의 켜져있을 때
							$(this).parents(".all_chk_container").find(".all_chk_div, .all_chk_div *").removeClass("on");
						}
					}
				});
			});

			$(".has_sub_chk_div input").each(function(){
				$(this).click(function(){
					if($(this).parents(".main_chk").size() > 0){
						if($(this).parents(".chk_box").hasClass("on")){
							$(this).parents(".has_sub_chk_div").find(".sub_chk").addClass("on");
						}else{
							$(this).parents(".has_sub_chk_div").find(".sub_chk").removeClass("on");
						}
					}else{
						if($(this).parents(".has_sub_chk_div").find(".sub_chk").hasClass("on")){
							if(!$(this).parents(".has_sub_chk_div").find(".main_chk").hasClass("on")){
								$(this).parents(".has_sub_chk_div").find(".main_chk").addClass("on")
							}
						}else{
							$(this).parents(".has_sub_chk_div").find(".main_chk").removeClass("on")
						}
					}
				})
			})


			//input X버튼
			var isMouseCheck = new Array();
			$(document).on("click", ".clear_has input", function(){
				if($(this).parents(".input_form.point").size() > 0) {
					if ($(this).val() != "0") {
						$(this).parents(".clear_has").find(".delete").show();
					}
				}else {
					if ($(this).val() != "") {
						$(this).parents(".clear_has").find(".delete").show();
					}
				}
			});
			$(document).on("keyup",".clear_has input", function(){
				if($(this).parents(".input_form.point").size() > 0) {
					if ($(this).val() == "0") {
						$(this).parents(".clear_has").find(".delete").hide();
					} else {
						$(this).parents(".clear_has").find(".delete").show();
					}
				}else {
					if ($(this).val() == "") {
						$(this).parents(".clear_has").find(".delete").hide();
					} else {
						$(this).parents(".clear_has").find(".delete").show();
					}
				}
			});
			$(document).on("click",".clear_has .delete", function(){
				if($(this).parents(".input_form.point").size() > 0) {
					$(this).parents(".clear_has").find("input").val("0");
				}else{
					$(this).parents(".clear_has").find("input").val("");
				}
				$(this).parents(".clear_has").find("input").removeClass("on");
				$(this).parents(".clear_has").find(".delete").hide();
				$(this).parents(".clear_has").find("input").focus();
			});

			$(document).on("mouseenter",".clear_has", function(){
				isMouseCheck[$(".clear_has").index(this)] = true;
			})

			$(document).on("mouseleave",".clear_has", function(){
				isMouseCheck[$(".clear_has").index(this)] = false;
			})

			$(document).on("focusout",".clear_has", function(){
				if (!isMouseCheck[$(".clear_has").index(this)]) {
					$(this).find(".delete").hide();
				}
			})

			// on/off btn 이벤트
			$(".on_off_btn_div").each(function(){
				$(this).find("a").click(function(){
					$(this).siblings("a").removeClass("on");
					$(this).addClass("on");
				})
			})

			//메인 팝업
			if($(".main_pop").size() > 0){
				$(".main_pop").stop(true, true).fadeIn(300);
				$(".dimd_bg").stop(true, true).fadeIn(300);

				if($(".main_pop .swiper-slide").size() > 1){
					//메인팝업 스와이퍼
					var mainPopSwiper = new Swiper($(".main_pop .swiper-container"), {
						observer: true,
						observeParents: true,
						pagination: {
							el: $(".main_pop .swiper-container .swiper-pagination"),
							type: "fraction",
						},
						loop: true,
						loopAdditionalSlides: 0,
					});
				}
				$(".main_pop .close").on("click", function(){
					$(".main_pop").stop(true, true).fadeOut(300);
					$(".dimd_bg").stop(true, true).fadeOut(300);
				})
			}


			//메인 스와이퍼
			var mainBannerSwiper = new Swiper($(".main_div .main_banner_div .swiper-container"), {
				observer: true,
				observeParents: true,
				pagination: {
					el: $(".main_div .main_banner_div .swiper-container .swiper-pagination"),
					type: "progressbar",
				},
				loop: true,
				loopAdditionalSlides: 0,
			});
			
			//고객리뷰 스와이퍼
			var customerReviewSwiper = new Swiper($(".main_div .customer_review_div .swiper-container"), {
				slidesPerView: "auto",
				spaceBetween: 8,
				observer: true,
				observeParents: true,
				pagination: {
					el: $(".main_div .customer_review_div .swiper-container .swiper-pagination"),
					type: "progressbar",
				},
			});

			//리뷰상세 팝업 이미지 스와이퍼
			$(".review_dtl_pop").each(function (q) {
				var reviewDtlPopSwiper = new Swiper($(this).find(".review_img_swiper"), {
					observer: true,
					observeParents: true,
					spaceBetween: 0,
					pagination: {
						el: $(this).find(".swiper-pagination"),
						type: 'fraction',
					},
					loop: true,
				});
			})

			//제품 전시 스와이퍼 타입
			$(".item_disp_type.func_swiper").each(function (q) {
				var gridSwiper = new Swiper($(this).find(".swiper-container"), {
					slidesPerView: "auto",
					spaceBetween: 4,
					observer: true,
					observeParents: true,
					pagination: {
						el: $(this).find(".swiper-pagination"),
						type: "progressbar",
					},
				});
			})


			/* 상품상세 */
			//썸네일
			if ($(".item_dtl_div .thumb_box .swiper-slide").size() > 1) {
				var productDtlSwiper = new Swiper(".item_dtl_div .thumb_box", {
					observer: true,
					observeParents: true,
					loop: true,
					pagination: {
						el: $(".item_dtl_div .thumb_box .swiper-pagination"),
						type: 'fraction',
					},
				});
			}
			
			$(".item_dtl_div .item_dtl_tab_area .anchor_tab a").each(function (q) {
				$(this).click(function () {
					$(this).parents(".anchor_tab").find("a").removeClass("on");
					$(this).addClass("on");
					TweenMax.to($("html, body"), 0.4, {scrollTop: $(".item_dtl_div .item_dtl_tab_area .tab_content .tab_con").eq(q).offset().top - $(".indicator").outerHeight() - $(".item_dtl_div .item_dtl_tab_area .anchor_tab").outerHeight() -headBannerH})
				})
			})


			//주문/결제 주문상품 아코디언
			$(".ord_div .section .tit_area.open").click(function(){
				if (!$(this).parents(".section").hasClass("on")) {
					$(this).parents(".section").addClass("on");
					$(this).parents(".section").find(".close").stop(true, true).slideDown(300);
				} else {
					$(this).parents(".section").removeClass("on");
					$(this).parents(".section").find(".close").stop(true, true).slideUp(300);
				}
			})


			//리뷰작성 별점
			$(".review_wrt_div .satisfaction .star .click").each(function (q) {
				$(this).click(function () {
					$(".review_wrt_div .satisfaction .star").removeClass().addClass("star");
					$(".review_wrt_div .satisfaction .star").addClass($(this).attr("class").split("click")[1].replace(" ", ""));

					$(".review_wrt_div .satisfaction .star .comment p").hide();
					$(".review_wrt_div .satisfaction .star .comment p").eq(q + 1).show();
				})
			})
			
			//리뷰 이미지 스와이퍼
			$(".review_lst_div .list").each(function (q) {
				var reviewDtlSwiper = new Swiper($(this).find(".review_img_swiper"), {
					observer: true,
					observeParents: true,
					spaceBetween: 0,
					pagination: {
						el: $(this).find(".swiper-pagination"),
						type: 'fraction',
					},
					loop: true,
				});
			})

			//faq 스와이퍼
			var faqCateSwiper = new Swiper($(".faq_cate_swiper"), {
				slidesPerView: "auto",
				spaceBetween: 8,
				observer: false,
				observeParents: false,
			});

			
			//1:1문의 이미지 스와이퍼
			$(".inqu_list_div .list").each(function (q) {
				var reviewDtlSwiper = new Swiper($(this).find(".inqu_img_swiper"), {
					observer: true,
					observeParents: true,
					spaceBetween: 0,
					pagination: {
						el: $(this).find(".swiper-pagination"),
						type: 'fraction',
					},
					loop: true,
				});
			})


			// on/off btn 이벤트
			$(".on_off_btn_div, .pay_way_div").each(function(){
				$(this).find("a").click(function(){
					$(this).siblings("a").removeClass("on");
					$(this).addClass("on");
				})
			})

			
			//body 클릭 이벤트
			$("body").click(function(e){
				//툴팁 끄기
				if($(".tooltip").is(":visible")) {
					if(!$(".tooltip.on").parents(".tooltip_div").has(e.target).length) {
						$(".tooltip.on").removeClass("on");
					}
					$(".tooltip").not(".on").parents(".tooltip_div").css({"z-index":"3"});
				}
				
				/* toolbox 끄기 */
				$(".tool_box").each(function(){
					if(!$(".tool_box").has(e.target).length) {
						if($(this).hasClass("on")){
							$(this).removeClass("on")
						}
					}
				})
			})

			//전시리스트 뷰타입
			$(".item_disp_div .view_type a").each(function(){
				$(this).click(function(){
					$(this).siblings("a").removeClass("on");
					$(this).addClass("on");
					if($(this).hasClass("full")){
						$(this).parents(".item_disp_div").find(".item_disp_list_div").addClass("full");
					}else{
						$(this).parents(".item_disp_div").find(".item_disp_list_div").removeClass("full");
					}
				});
			})
			
			$(".sort_select_pop .list").click(function(){
				$(this).addClass("on");
				$(this).siblings(".list").removeClass("on");
			})
			
			//전시리스트 스와이퍼
			var itemCateSwiper1 = new Swiper($(".item_disp_div .top_tab_swiper"), {
				slidesPerView: "auto",
				spaceBetween: 42,
				observer: false,
				observeParents: false,
			});

			var itemCateSwiper2 = new Swiper($(".item_disp_div .category_swiper"), {
				slidesPerView: "auto",
				spaceBetween: 8,
				observer: false,
				observeParents: false,
			});


			//상품상세 옵션 레이어 이벤트
			$(".item_option_layer a").click(function(){
				if(!$(this).hasClass("option_close")){
					if(!$(this).parents(".item_option_layer").hasClass("on")){
						scrollT = $(window).scrollTop();
						$(this).parents(".item_option_layer").addClass("on");
						$(".dimd_bg").stop(true, true).fadeIn(300);
						$("html").addClass("lock");
						$(this).parents(".item_option_layer").find(".option_dtl").stop(true, true).slideDown(200);
					}
				}else{
					$(this).parents(".item_option_layer").removeClass("on");
					$(".dimd_bg").stop(true, true).fadeOut(300);
					$("html").removeClass("lock");
					$("html, body").scrollTop(scrollT);
					$(this).parents(".item_option_layer").find(".option_dtl").stop(true, true).slideUp(200);
				}

			})

			//dimd 클릭시 옵션 레이어 닫기 이벤트
			$(".dimd_bg").click(function (){
				if($(".item_option_layer").hasClass("on")){
					$(".item_option_layer .option_close").click();
				}
			});

			/* toolbox */
			$(".tool_box").each(function(){
				$(this).find(".tool_open").click(function(){
					$(this).parents(".tool_box").toggleClass("on")
				});
			}) 

			/* tooltip */
			$(document).on("click", ".opener", function(e){
				var windowWidth = window.innerWidth;
				var windowHeight = window.innerHeight;
				var openerL = $(this).offset().left;
				var openerT = $(this).offset().top - $(window).scrollTop();
				var openerW = $(this).outerWidth();
				var openerH = $(this).outerHeight();
				var tooltipW = $(this).parents(".tooltip_div").find(".tooltip").outerWidth();
				var tooltipH = $(this).parents(".tooltip_div").find(".tooltip").outerHeight();
				// 레이어가 나타날 위치
				var tooltipPosL = 0;
				var tooltipPosT = 0;


				//LEFT
				if (openerL + tooltipW >= windowWidth) {
					if(tooltipW - openerW - openerL > 0){
						tooltipPosL = parseInt(windowWidth/2 - openerL - tooltipW/2);
					}else{
						tooltipPosL = (openerW - tooltipW);
					}
				} else {
					tooltipPosL = 0;
				}

				//TOP
				if (openerT + openerH + tooltipH >= windowHeight) {
					tooltipPosT = -(tooltipPosT + tooltipH);
				} else {
					tooltipPosT = openerH;
				}

				if($(this).parents(".tooltip_div").find(".tooltip").hasClass("on")){
					$(this).parents(".tooltip_div").find(".tooltip").removeClass("on");
					$(".opener").not(this).parents(".tooltip_div").css({"z-index":"3"});
				}else{
					$(".tooltip").removeClass("on");
					$(this).parents(".tooltip_div").find(".tooltip").addClass("on");
					$(this).parents(".tooltip_div").find(".tooltip").css({"top": tooltipPosT, "left": tooltipPosL});
					$(".opener").not(this).parents(".tooltip_div").css({"z-index":"1"});

				}

				$(".tooltip").each(function () {
					$(this).click(function (s) {
						s.stopPropagation();
					});
				});
			});

			/* 전체메뉴 앵커이동 */
			$(".cate_pop .anchor_div .anchor_tab a").each(function (q) {
				$(this).click(function (e) {
					$(this).addClass("on");
					$(this).siblings().removeClass("on");
					var obj = $(".cate_pop .anchor_wrap .menu").eq(q).find(".title");
					var scTop = obj.position().top + 168;
					TweenMax.to($(".cate_wrap"), 0.3, {scrollTop: scTop, ease: Power3.easeOut});
				});
			})

			//뷰타입
			$(".product_list_top a").each(function (q) {
				$(this).click(function () {
					if (!$(this).hasClass("on")) {
						$(".product_list_top a").removeClass("on");
						$(this).addClass("on");

						if (!q) {
							$(".product_list_div").removeClass("hor");
						} else {
							$(".product_list_div").addClass("hor");
						}
					}
				})
			})

			//select textarea
			$("select").each(function(){
				$(this).change(function(){
					if($(this).val() == "textarea"){
						$(this).parents(".input_form").find(".textarea_div").parents(".input_area").show();
					}else{
						$(this).parents(".input_form").find(".textarea_div").parents(".input_area").hide();
					}
				})
			})

			//주문배송조회 검색
			$(".order_div .search_area input").focusin(function () {
				$("html").addClass("lock");
				$(".order_div .order_dimd").stop(true, true).fadeIn(300);
				$(".order_div .search_area .hide_con").stop(true, true).slideDown(300);
			});
			$(".order_div .order_dimd").click(function () {
				$("html").removeClass("lock");
				$(".order_div .order_dimd").stop(true, true).fadeOut(300);
				$(".order_div .search_area .hide_con").stop(true, true).slideUp(300);
			});
			$(".order_div .search_area .detail .btn_div").each(function () {
				$(this).find("a").each(function (q) {
					$(this).click(function () {
						if (!$(this).hasClass("on")) {
							$(this).parents(".btn_div").find("a").removeClass("on");
							$(this).addClass("on");
							$(".order_div .search_area .detail .select_date").removeClass("on");
						}
					})
				})
			})
			$(".order_div .search_area .detail .select_date").click(function(){
				$(".order_div .search_area .detail .btn_div a").removeClass("on");
				$(this).addClass("on");
			})

			//개인정보처리방침 앵커
			if($(".terms_div").size () > 0) {
				if ($(".terms_div").parents(".layer_pop").size() == 0) {
					for (i = 0; i < $(".terms_div .text_div .tit").not(".etc").size(); i++) {
						$(".terms_div .anchor_div").append('<h2><a href="javascript:">' + $(".terms_div .text_div .tit").eq(i).text() + '</a></h2>');
					}
					$(document).on("click", ".terms_div .anchor_div a", function () {
						TweenMax.to($("html, body"), 0.5, {scrollTop: $(".terms_div .text_div .tit").eq($(this).parents("h2").index()).offset().top - 24})
					})
				}
			}

			$(document).on("click", ".layer_pop .close", function(){
				if(!$(this).parents(".accor_div").size() > 0) {
					$(this).parents(".layer_pop").stop(true, true).fadeOut(300);
				}
					history.back();
			})

			//이벤트 상세 다른 이벤트 스와이퍼
			if($(".event_dtl_div .other_list_area .list_div .list").size() > 1){
				$(".event_dtl_div .other_list_area .list_div").addClass("swiper-container");
				var evtOtherSwiper = new Swiper($(".event_dtl_div .swiper-container"), {
					slidesPerView: "auto",
					spaceBetween: 12,
					observer: false,
					observeParents: false,
				});
			}

		},

		layerPopEvt: function (popName, depth) {
			history.pushState(null, document.title, location.href);
			historyCnt++;

			//팝업이벤트
			//alert팝업, confirm팝업, toast팝업은 개별 함수참고
			var scrollT = 0;
			$("." + popName).stop(true, true).fadeIn(300);
			scrollT = $(window).scrollTop();
			if (depth == "y") {
				$("." + popName).css("z-index","16");
				$(".dimd_bg").css("z-index","15");
			}
			$(".dimd_bg").stop(true, true).fadeIn(300);
			$("html").addClass("lock");
			$("html").height($("#wrap").outerHeight())

			//닫기
			$("." + popName + " .close").on("click", function () {
				if(!$(this).parents(".accor_div").size() > 0) {
					if (depth == "y") {
						$(".dimd_bg").css("z-index", "");
					} else {
						setTimeout(function(){
							if($(".alert_pop").size() == 0){
								$(".dimd_bg").stop(true, true).fadeOut(300);
							}
						},200)
						$("html").removeClass("lock");
						$("html").height("");
						$("html, body").scrollTop(scrollT);
					}

				}
			});

			//뒤로가기
			window.onpopstate = function(event){
				popCnt ++;
				if ($("."+popName).is(':visible')){
					if(!$(this).parents(".accor_div").size() > 0) {
						$("." + popName).stop(true, true).fadeOut(300);
						if (depth == "y") {
							$(".dimd_bg").css("z-index", "");
						} else {
							if($(".alert_pop").size() == 0){
								$(".dimd_bg").stop(true, true).fadeOut(300);
							}
							$("html").removeClass("lock");
							$("html").height("");
							$("html, body").scrollTop(scrollT);
						}
					}
					history.scrollRestoration = 'manual';
				}else{
					history.go(-(historyCnt - popCnt+1));
				}
			}

			//팝업 내 스와이퍼 이미지 리사이즈
			if($("." + popName).find(".swiper-container").size() > 0){
				console.log(1);
				setTimeout(() => {
					afterResize(".swiper-container .img_resize")
				}, 100);
			}

		},
	}
})();

$(document).ready(function () {
	mSubScript.checkEvt();
	mSubScript.selectEvt();
	mSubScript.tabEvt();
	mSubScript.accorEvt();
	mSubScript.resizeEvt();
	mSubScript.scrollEvt();
	mSubScript.etcEvt();
});

$(window).load(function () {
	mSubScript.resizeEvt();
});

function optionLayerFunc(thisLayer){
	//옵션레이어 상품선택창 max-height값
	var sumH = 0;
	for(var i=0; i < thisLayer.find(".tit").length; i++) {
		sumH += thisLayer.find(".tit").eq(i).outerHeight(true);
		sumH += thisLayer.find(".select_box").eq(i).outerHeight(true);
	}
	sumH += thisLayer.find(".bottom_btn").outerHeight(true);
	thisLayer.find(".select_box .two_depth").css("max-height", $(window).height() - parseInt(thisLayer.css("padding-top").split("p")[0]) - parseInt(thisLayer.css("padding-bottom").split("p")[0]) - 100 - (sumH - thisLayer.find(".select_box.on .two_depth").outerHeight()))
}

function afterResize(selector) {
	//이미지 로드 후 리사이징, 비동기 처리 부분
	$(selector).each(function () {
		if ($(this).find("img").size() > 0) {
			if($(this).parents(".swiper-container").size() > 0){
				if ($(this).find("img").get(0).naturalWidth * $(this).height() / ($(this).find("img").get(0).naturalHeight * $(this).width()) >= 1){
					$(this).addClass("reverse");
				}
			}else{
				if ($(this).find("img").get(0).naturalWidth * $(this).height() / ($(this).find("img").get(0).naturalHeight * $(this).width()) <= 1){
					$(this).addClass("reverse");
				}
			}
		}
	});
}

function alertPop(tit, txt, txt2, btn, url, depth) { //txt : 서브텍스트, btn : 확인버튼 문구, url : 확인버튼 url 추가시
	//확인버튼만 있는 알럿팝업
	if (depth == "y") {
		var scrollT = 0;
	}

	//url 입력안할시
	if (url == undefined || url == "") {
		url = 'javascript:';
	} else if (url == "reload") {
		url = 'javascript:location.reload();';
	} else if (url == "back"){
		url = 'javascript:history.back();';
	};

	$("#wrap").append('<div class="alert_pop"><div class="pop_con"><p class="tit">' + tit + '</p><p class="txt">' + txt + '</p><p class="txt2">' + txt2 + '</p><div class="pop_btn"><a href="' + url + '" class="ok_btn close">' + btn + '</a></div></div></div>');
	if (tit == undefined || tit == "") {
		$(".alert_pop .pop_con .tit").remove();
	}
	if (txt == undefined || txt == "") {
		$(".alert_pop .pop_con .txt").remove();
	}
	if (txt2 == undefined || txt2 == "") {
		$(".alert_pop .pop_con .txt2").remove();
	}



	if ($(".alert_pop").width() % 2 != 0) {
		$(".alert_pop").width($(".alert_pop").width() - 1);
	}
	$(".alert_pop").stop(true, true).fadeIn(300);
	if (depth == "y") {
		$(".dimd_bg").css("z-index", "15");
	}else{
		$("html").addClass("not_scroll");
	}
	$(".dimd_bg").stop(true, true).fadeIn(300);
	/*
	scrollT = $(window).scrollTop();
	$("html").addClass("lock");
	$("html").height($("#wrap").outerHeight());
	 */

	$(".alert_pop .close").on("click", function () {
		$(".alert_pop").stop().fadeOut(300, function () {
			$(this).remove();
		});
		if (depth == "y") {
			$(".dimd_bg").css("z-index", "");
		} else {
			$("html").removeClass("not_scroll");
			$(".dimd_bg").stop().fadeOut(300);
			/*
			$("html").removeClass("lock");
			$("html").height("");
			$("html, body").scrollTop(scrollT);
			*/
		}
	});
}

function confirmPop(tit, txt, txt2, no, ok, noId, okId, depth) { //txt : 서브텍스트, ok : 확인버튼 문구, no : 취소버튼 문구, okId : 확인버튼 id값(이벤트 처리위해 등록), noId : 취소버튼 id값(이벤트 처리위해 등록)
	//두가지 버튼 지닌 컨펌팝업
	var scrollT = 0;

	$("#wrap").append('<div class="alert_pop"><div class="pop_con"><p class="tit">' + tit + '</p><p class="txt">' + txt + '</p><p class="txt2">' + txt2 + '</p><div class="pop_btn multiple"><a href="javascript:" id="' + noId + '" class="no_btn close">' + no + '</a><a href="javascript:" id="' + okId + '" class="ok_btn">' + ok + '</a></div></div></div>');
	if (tit == undefined || tit == "") {
		$(".alert_pop .pop_con .tit").remove();
	}
	if (txt == undefined || txt == "") {
		$(".alert_pop .pop_con .txt").remove();
	}
	if (txt2 == undefined || txt2 == "") {
		$(".alert_pop .pop_con .txt2").remove();
	}

	if ($(".alert_pop").width() % 2 != 0) {
		$(".alert_pop").width($(".alert_pop").width() - 1);
	}
	$(".alert_pop").stop(true, true).fadeIn(300);
	if (depth == "y") {
		$(".dimd_bg").css("z-index", "15");
	}else{
		$("html").addClass("not_scroll");
	}
	$(".dimd_bg").stop(true, true).fadeIn(300);
	/*
	scrollT = $(window).scrollTop();
	$("html").addClass("lock");
	$("html").height($("#wrap").outerHeight());
	*/
	$(".alert_pop .close").on("click", function () {
		$(".alert_pop").stop().fadeOut(300, function () {
			$(this).remove();
		});
		if (depth == "y") {
			$(".dimd_bg").css("z-index", "");
		} else {
			$("html").removeClass("not_scroll");
			$(".dimd_bg").stop().fadeOut(300);
			/*
			$("html").removeClass("lock");
			$("html").height("");
			$("html, body").scrollTop(scrollT);
			 */
		}
	});
}

function toastPop(tit) {
	if ($("#wrap").find(".toast_pop").size() > 0) {
		$(".toast_pop").remove();
	}
	$("#wrap").append('<div class="toast_pop"><div class="pop_con"><p class="title">' + tit + '</p></div></div>');
	if ($(".toast_pop").width() % 2 != 0) {
		$(".toast_pop").width($(".toast_pop").width() - 1);
	}
	TweenMax.to($(".toast_pop"), 0.3, {
		bottom: 40, ease: Power3.easeOut, onComplete: function () {
			TweenMax.to(this.targets(), 0.3, {
				bottom: -200, delay: 2, ease: Power3.easeIn, onComplete: function () {
					this.targets()[0].remove();
					isToastMake = true;
				}
			})
		}
	})
}
